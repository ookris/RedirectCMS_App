...
<head>
  ...
  <style>
    ...
    .site-nav .nav-link:hover, .site-nav .nav-link.active { color: var(--theme-primary, #4a90d9); border-bottom-color: var(--theme-primary, #4a90d9); }

    /* FILTER BAR */
    .filter-bar { background: #fff; padding: 12px 0; border-bottom: 1px solid #e9ecef; }
    .filter-pill { display: inline-flex; align-items: center; gap: 4px; padding: 4px 12px; border-radius: 20px; font-size: .78rem; font-weight: 600; border: 1.5px solid transparent; cursor: pointer; text-decoration: none; transition: background var(--transition), color var(--transition), border-color var(--transition); white-space: nowrap; }
    .filter-pill.fp-all       { background: var(--theme-primary, #4a90d9); color: #fff; }
    .filter-pill.fp-all.inactive { background: #f0f0f0; color: #555; border-color: #ddd; }
    .filter-pill.fp-tag.active   { background: var(--theme-accent, #e83e8c); color: #fff; }
    .filter-pill.fp-tag          { color: #777; border-color: #ddd; }
    .filter-pill.fp-tag:hover    { border-color: var(--theme-accent, #e83e8c); color: var(--theme-accent, #e83e8c); }
    .active-filter-bar { background: #fffbeb; border-bottom: 1px solid #fef08a; padding: 8px 0; font-size: .85rem; }

    /* CARDS */
    .post-card { border: none; border-radius: 12px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,.07); height: 100%; transition: transform var(--transition), box-shadow var(--transition); background: #fff; }
    .post-card:hover { transform: translateY(-4px); box-shadow: 0 8px 24px rgba(0,0,0,.12); }
    .post-card-img { height: 192px; object-fit: cover; width: 100%; }
    .post-card-placeholder { height: 192px; display: flex; align-items: center; justify-content: center; font-size: 3.5rem; font-weight: 800; letter-spacing: -2px; }
    .post-card-body { padding: 16px; display: flex; flex-direction: column; flex: 1; }
    .post-card-title { font-size: 1rem; font-weight: 700; line-height: 1.4; margin-bottom: 6px; }
    .post-card-title a { color: #1a202c; text-decoration: none; }
    .post-card-title a:hover { color: var(--theme-primary, #4a90d9); }
    .post-card-desc { font-size: .84rem; color: #64748b; line-height: 1.6; flex-grow: 1; }
    .post-card-meta { font-size: .75rem; color: #94a3b8; }
    .cat-badge { display: inline-block; padding: 2px 8px; border-radius: 10px; font-size: .7rem; font-weight: 700; color: #fff; white-space: nowrap; }
    .read-more-btn { font-size: .8rem; font-weight: 600; color: var(--theme-primary, #4a90d9); text-decoration: none; }
...
<nav class="site-nav">
  <div class="container">
    <div class="d-flex flex-wrap">
      <a href="<?= $basePath ?>/" class="nav-link active">Blog</a>
...
<div class="filter-bar">
  <div class="container">
    <div class="d-flex flex-wrap gap-2 align-items-center">
      <?php $noFilter = empty($_GET['category']) && empty($_GET['tag']); ?>
      <a href="<?= $basePath ?>/" class="filter-pill fp-all <?= $noFilter ? '' : 'inactive' ?>">Wszystkie</a>

      <?php foreach ($allCategories as $_cat): ?>
        <?php $isActive = !empty($_GET['category']) && $_GET['category'] === $_cat['slug']; ?>
        <?php $cc = htmlspecialchars($_cat['color'] ?? '#6c757d'); ?>
        <a href="<?= !empty($prettyUrls) ? $basePath . '/category/' . urlencode($_cat['slug']) : $basePath . '/?category=' . urlencode($_cat['slug']) ?>"
           ...
           style="<?= $isActive ? "background:{$cc};color:#fff;border-color:{$cc};" : "color:{$cc};border-color:{$cc}40;" ?>">
          <?= htmlspecialchars($_cat['name']) ?><?php if (!empty($_cat['post_count'])): ?> <span style="opacity:.6;font-weight:400"><?= (int)$_cat['post_count'] ?></span><?php endif; ?>
        </a>
      <?php endforeach; ?>

      <?php if (!empty($allTags)): ?>
        <div class="vr mx-1 d-none d-sm-block" style="opacity:.25"></div>
        <?php foreach ($allTags as $_tag): ?>
          <?php $isTagActive = !empty($_GET['tag']) && $_GET['tag'] === $_tag['slug']; ?>
          <a href="<?= !empty($prettyUrls) ? $basePath . '/tag/' . urlencode($_tag['slug']) : $basePath . '/?tag=' . urlencode($_tag['slug']) ?>"
             class="filter-pill fp-tag <?= $isTagActive ? 'active' : '' ?>">
  ...
  <div class="container d-flex align-items-center justify-content-between">
    <span>
      ...
      <?php if (!empty($activeCategory)): ?>
        <strong style="color:<?= htmlspecialchars($activeCategory['color'] ?? '#333') ?>"><?= htmlspecialchars($activeCategory['name']) ?></strong>
      <?php else: ?>
        <strong>#<?= htmlspecialchars($activeTag['name']) ?></strong>
      <?php endif; ?>
    </span>
    <a href="<?= $basePath ?>/" class="text-secondary small">✕ Wyczyść filtr</a>
  </div>
</div>
<?php endif; ?>
    ...
    <div class="col-lg-8">
      <?php if (empty($blogPosts)): ?>
        <div class="text-center py-5 text-muted">
          <svg xmlns="http://www.w3.org/2000/svg" width="52" height="52" fill="currentColor" class="bi bi-journal-x mb-3 d-block mx-auto opacity-25" viewBox="0 0 16 16">
            <path fill-rule="evenodd" d="M6.146 6.146a.5.5 0 0 1 .708 0L8 7.293l1.146-1.147a.5.5 0 1 1 .708.708L8.707 8l1.147 1.146a.5.5 0 0 1-.708.708L8 8.707l-1.146 1.147a.5.5 0 0 1-.708-.708L7.293 8 6.146 6.854a.5.5 0 0 1 0-.708"/>
          ...
          <p class="mb-3">Brak wpisów do wyświetlenia.</p>
          <?php if (!empty($activeCategory) || !empty($activeTag)): ?>
            <a href="<?= $basePath ?>/" class="btn btn-sm btn-outline-secondary">Pokaż wszystkie wpisy</a>
          <?php endif; ?>
        </div>
      <?php else: ?>
        <div class="row row-cols-1 row-cols-sm-2 g-4">
          <?php foreach ($blogPosts as $_post): ?>
            <?php
              ...
              $imgSrc  = $hasImg ? $basePath . '/' . ltrim((string)$_post['og_image'], '/') : null;
              $firstLtr = mb_strtoupper(mb_substr((string)($_post['page_title'] ?? $_post['slug'] ?? 'P'), 0, 1));
              $bgColor = htmlspecialchars($_post['category_color'] ?? '#7952b3');
              $stripped = strip_tags((string)($_post['page_description'] ?? ''));
              $desc    = mb_strlen($stripped) > $blogDescLength
                         ? mb_substr($stripped, 0, $blogDescLength) . '…'
                         : $stripped;
            ?>
            <div class="col d-flex">
              <div class="post-card d-flex flex-column w-100">
                <?php if ($hasImg): ?>
                  <img src="<?= htmlspecialchars($imgSrc) ?>"
                       alt="<?= htmlspecialchars($_post['page_title'] ?? '') ?>"
                       class="post-card-img" loading="lazy" />
                <?php else: ?>
                  <div class="post-card-placeholder" style="background:<?= $bgColor ?>18;color:<?= $bgColor ?>;">
                    <?= htmlspecialchars($firstLtr) ?>
                  </div>
                <?php endif; ?>
                <div class="post-card-body">
                  ...
                  <?php if ($desc !== ''): ?>
                    <p class="post-card-desc mb-0"><?= htmlspecialchars($desc) ?></p>
                  <?php endif; ?>
                  <div class="d-flex align-items-center justify-content-between mt-auto pt-3">
                    ...
                    <a href="<?= $basePath ?>/blog/<?= htmlspecialchars($_post['slug']) ?>" class="read-more-btn">
                      Czytaj więcej →
                    </a>
                  </div>
                </div>
      ...
      <?php if (!empty($totalPages) && $totalPages > 1): ?>
        <?php
          ...
          if (isset($_GET['category']) && is_string($_GET['category']) && $_GET['category'] !== '') {
            $_pParams['category'] = $_GET['category'];
          }
          if (isset($_GET['tag']) && is_string($_GET['tag']) && $_GET['tag'] !== '') {
            $_pParams['tag'] = $_GET['tag'];
          }
          $_pBase      = $basePath . '/?' . (!empty($_pParams) ? http_build_query($_pParams) . '&' : '');
          $_cp         = (int)$currentPage;
          $_tp         = (int)$totalPages;
          $_prevDis    = ($_cp <= 1);
          $_nextDis    = ($_cp >= $_tp);
          $_prevPage   = $_prevDis ? 1 : $_cp - 1;
          $_nextPage   = $_nextDis ? $_tp : $_cp + 1;
          $_win        = 2;
          $_start      = max(1, $_cp - $_win);
          $_end        = min($_tp, $_cp + $_win);
          if ($_cp <= $_win + 1)    { $_start = 1;   $_end = min($_tp, 1 + 2 * $_win); }
          elseif ($_cp >= $_tp - $_win) { $_end = $_tp; $_start = max(1, $_tp - 2 * $_win); }
        ?>
        <nav aria-label="Nawigacja strony" class="mt-4">
          <ul class="pagination justify-content-center flex-wrap">
            <li class="page-item <?= $_prevDis ? 'disabled' : '' ?>">
              <?php if ($_prevDis): ?>
                <span class="page-link" aria-disabled="true" tabindex="-1">&laquo;</span>
              <?php else: ?>
                <a class="page-link" href="<?= htmlspecialchars($_pBase . 'p=' . $_prevPage, ENT_QUOTES) ?>" aria-label="Poprzednia">&laquo;</a>
              <?php endif; ?>
            </li>
            <?php if ($_start > 1): ?>
              <li class="page-item <?= 1 === $_cp ? 'active' : '' ?>">
                <a class="page-link" href="<?= htmlspecialchars($_pBase . 'p=1', ENT_QUOTES) ?>">1</a>
              </li>
              <?php if ($_start > 2): ?>
                <li class="page-item disabled"><span class="page-link">&hellip;</span></li>
              <?php endif; ?>
            <?php endif; ?>
            <?php for ($_pn = $_start; $_pn <= $_end; $_pn++): ?>
              <li class="page-item <?= $_pn === $_cp ? 'active' : '' ?>">
                <a class="page-link" href="<?= htmlspecialchars($_pBase . 'p=' . $_pn, ENT_QUOTES) ?>"><?= $_pn ?></a>
              </li>
            <?php endfor; ?>
            <?php if ($_end < $_tp): ?>
              <?php if ($_end < $_tp - 1): ?>
                <li class="page-item disabled"><span class="page-link">&hellip;</span></li>
              <?php endif; ?>
              <li class="page-item <?= $_tp === $_cp ? 'active' : '' ?>">
                <a class="page-link" href="<?= htmlspecialchars($_pBase . 'p=' . $_tp, ENT_QUOTES) ?>"><?= $_tp ?></a>
              </li>
            <?php endif; ?>
            <li class="page-item <?= $_nextDis ? 'disabled' : '' ?>">
              <?php if ($_nextDis): ?>
                <span class="page-link" aria-disabled="true" tabindex="-1">&raquo;</span>
              <?php else: ?>
                <a class="page-link" href="<?= htmlspecialchars($_pBase . 'p=' . $_nextPage, ENT_QUOTES) ?>" aria-label="Następna">&raquo;</a>
              <?php endif; ?>
            </li>
          </ul>
        </nav>
