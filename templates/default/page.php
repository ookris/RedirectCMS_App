...
<head>
  ...
  <style>
    ...
    /* PAGE CONTENT */
    .page-content { background: #fff; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,.08); padding: 40px; }
    .page-content img { max-width: 100%; height: auto; border-radius: 8px; }
    .page-content h1, .page-content h2, .page-content h3 { color: #1a1a2e; }
    .page-content a { color: var(--theme-primary, #7952b3); }
    ...
    <div class="d-flex flex-wrap">
      ...
      <?php foreach ($navPages as $_np): ?>
        <a href="<?= $basePath ?>/?page=<?= htmlspecialchars($_np['slug']) ?>"
           class="nav-link <?= $_np['slug'] === ($currentSlug ?? '') ? 'active' : '' ?>">
          <?= htmlspecialchars($_np['title']) ?>
        </a>
      <?php endforeach; ?>
      <?php if (!empty($contactEnabled)): ?>
        <a href="<?= $basePath ?>/?page=contact"
           class="nav-link <?= ($currentSlug ?? '') === 'contact' ? 'active' : '' ?>">Kontakt</a>
      <?php endif; ?>
    </div>
  </div>
</nav>
