<?php
// _sidebar.php — Pixel theme sidebar widgets
// Variables available: $sidebarData, $basePath, $prettyUrls
// Uses px_* helpers defined in parent template (home.php / post.php)
...
<div class="px-widget">
  <?php if (!empty($_swTitle)): ?>
    <div class="px-widget-head"><?= htmlspecialchars($_swTitle) ?></div>
  <?php endif; ?>

  <?php if ($_swType === 'popular_posts' || $_swType === 'random_posts'): ?>
    <div class="px-widget-body" style="padding:0">
      <?php foreach ($_swData as $_i => $_wp): ?>
        <a href="<?= px_post_url($basePath, $_wp['slug'], $prettyUrls ?? false) ?>"
           style="display:flex;align-items:flex-start;gap:10px;padding:10px 16px;border-bottom:1px solid #f5f5f5;text-decoration:none;color:#2e2e2e">
          <!-- Number badge -->
          <span style="flex-shrink:0;width:22px;height:22px;border-radius:3px;background:<?= $_i === 0 ? 'var(--theme-primary,#2942ee)' : '#f0f0f0' ?>;color:<?= $_i === 0 ? '#fff' : '#8e8e95' ?>;font-family:'Josefin Sans',sans-serif;font-size:.7rem;font-weight:700;display:flex;align-items:center;justify-content:center;margin-top:1px">
            <?= $_i + 1 ?>
          </span>
          <span style="flex:1;min-width:0">
            <span style="display:block;font-family:'Josefin Sans',sans-serif;font-size:.8rem;font-weight:700;line-height:1.4;color:#2e2e2e">
              <?= htmlspecialchars($_wp['page_title']) ?>
            </span>
            <?php if (!empty($_wp['click_count'])): ?>
              <span style="font-size:.68rem;color:#8e8e95;margin-top:2px;display:block">
      ...
      <?php if (empty($_swData)): ?>
        <div style="padding:12px 16px;font-size:.8rem;color:#8e8e95">Brak wpisów</div>
      <?php endif; ?>
    </div>
    ...
    <div class="px-widget-body">
      <?php foreach ($_swData as $_tag): ?>
        <a href="<?= px_tag_url($basePath, $_tag['slug'], $prettyUrls ?? false) ?>"
           style="display:inline-block;padding:3px 9px;margin:3px 2px;border-radius:3px;font-size:.72rem;font-family:'Josefin Sans',sans-serif;font-weight:600;letter-spacing:.03em;background:#f0f0f0;color:#555;text-decoration:none">
          #<?= htmlspecialchars($_tag['name']) ?>
        </a>
      <?php endforeach; ?>
      <?php if (empty($_swData)): ?>
        <span style="font-size:.8rem;color:#8e8e95">Brak tagów</span>
      <?php endif; ?>
    </div>

  <?php elseif ($_swType === 'categories'): ?>
    <div class="px-widget-body" style="padding:0">
      <?php foreach ($_swData as $_i => $_cat): ?>
        <a href="<?= px_cat_url($basePath, $_cat['slug'], $prettyUrls ?? false) ?>"
          ...
          <span style="display:flex;align-items:center;gap:8px">
            <span style="width:8px;height:8px;border-radius:50%;background:<?= htmlspecialchars($_cat['color'] ?: '#aaa', ENT_QUOTES) ?>;flex-shrink:0"></span>
            <?= htmlspecialchars($_cat['name']) ?>
          </span>
          <span style="font-size:.7rem;color:#8e8e95;background:#f5f5f5;padding:1px 7px;border-radius:10px">
            <?= (int)$_cat['post_count'] ?>
          </span>
        </a>
      <?php endforeach; ?>
      <?php if (empty($_swData)): ?>
        <div style="padding:12px 16px;font-size:.8rem;color:#8e8e95">Brak kategorii</div>
      <?php endif; ?>
    </div>
    ...
    <div class="px-widget-body">
      ...
      ?>
        <a href="<?= htmlspecialchars($url, ENT_QUOTES) ?>" target="_blank" rel="noopener noreferrer"
           style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid #f5f5f5;text-decoration:none;color:#2e2e2e;font-family:'Josefin Sans',sans-serif;font-size:.8rem;font-weight:600">
          <span style="width:28px;height:28px;border-radius:4px;background:<?= htmlspecialchars($_def['color'], ENT_QUOTES) ?>;color:#fff;display:flex;align-items:center;justify-content:center;flex-shrink:0">
            <?= $_def['svg'] ?>
          </span>
          <?= htmlspecialchars($_def['label']) ?>
          <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" fill="currentColor" viewBox="0 0 16 16" style="margin-left:auto;color:#ccc"><path fill-rule="evenodd" d="M4.646 1.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1 0 .708l-6 6a.5.5 0 0 1-.708-.708L10.293 8 4.646 2.354a.5.5 0 0 1 0-.708z"/></svg>
        </a>
      <?php endforeach; ?>
      <?php if (!$hasSocialLink): ?>
        <span style="font-size:.8rem;color:#8e8e95">Brak skonfigurowanych linków</span>
      <?php endif; ?>
    </div>
  ...
  <?php else: ?>
    <div class="px-widget-body">
      <span style="font-size:.8rem;color:#8e8e95">Nieznany typ widgetu: <?= htmlspecialchars($_swType) ?></span>
    </div>
  <?php endif; ?>
</div>
<?php endforeach; ?>
