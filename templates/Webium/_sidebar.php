<?php
// _sidebar.php — Webium theme sidebar widgets
// Variables available: $sidebarData, $basePath, $prettyUrls
// Uses wb_* helpers defined in parent template (home.php / post.php)
...
<div class="wb-widget">
  <?php if (!empty($_swTitle)): ?>
    <div class="wb-widget-title"><?= htmlspecialchars($_swTitle) ?></div>
  <?php endif; ?>

  <?php if ($_swType === 'popular_posts' || $_swType === 'random_posts'): ?>
    <?php foreach ($_swData as $_i => $_wp): ?>
      <a href="<?= wb_post_url($basePath, $_wp['slug'], $prettyUrls ?? false) ?>"
         style="display:flex;align-items:flex-start;gap:10px;padding:10px 0;border-bottom:1px solid var(--theme-border,#e6e6e6);text-decoration:none;color:var(--theme-text,#292929)">
        <span style="flex-shrink:0;width:22px;height:22px;border-radius:50%;background:<?= $_i === 0 ? 'var(--theme-primary,#ffc017)' : 'var(--theme-chip-bg,#f2f2f2)' ?>;color:#000;font-size:.7rem;font-weight:700;display:flex;align-items:center;justify-content:center">
          <?= $_i + 1 ?>
        </span>
        <span style="flex:1;min-width:0">
          <span style="display:block;font-size:.85rem;font-weight:600;line-height:1.4;color:var(--theme-text,#292929)">
            <?= htmlspecialchars($_wp['page_title']) ?>
          </span>
          <?php if (!empty($_wp['click_count'])): ?>
            <span style="font-size:.72rem;color:var(--theme-muted,#757575);margin-top:2px;display:block">
    ...
    <?php if (empty($_swData)): ?>
      <span style="font-size:.8rem;color:var(--theme-muted,#757575)">Brak wpisów</span>
    <?php endif; ?>

  <?php elseif ($_swType === 'tag_cloud'): ?>
    <?php foreach ($_swData as $_tag): ?>
      <a href="<?= wb_tag_url($basePath, $_tag['slug'], $prettyUrls ?? false) ?>"
         class="wb-chip" style="display:inline-block;margin:3px 2px;font-size:.78rem">
        #<?= htmlspecialchars($_tag['name']) ?>
      </a>
    <?php endforeach; ?>
    <?php if (empty($_swData)): ?>
      <span style="font-size:.8rem;color:var(--theme-muted,#757575)">Brak tagów</span>
    <?php endif; ?>
  ...
  <?php elseif ($_swType === 'categories'): ?>
    <?php foreach ($_swData as $_cat): ?>
      <a href="<?= wb_cat_url($basePath, $_cat['slug'], $prettyUrls ?? false) ?>"
        ...
        <span style="font-size:.72rem;color:var(--theme-muted,#757575);background:var(--theme-chip-bg,#f2f2f2);padding:1px 8px;border-radius:99px">
          <?= (int)$_cat['post_count'] ?>
        </span>
      </a>
    <?php endforeach; ?>
    <?php if (empty($_swData)): ?>
      <span style="font-size:.8rem;color:var(--theme-muted,#757575)">Brak kategorii</span>
    <?php endif; ?>
  ...
  <?php elseif ($_swType === 'social_links'): ?>
    ...
    ?>
      <a href="<?= htmlspecialchars($url, ENT_QUOTES) ?>" target="_blank" rel="noopener noreferrer"
         style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--theme-border,#e6e6e6);text-decoration:none;color:var(--theme-text,#292929);font-size:.85rem;font-weight:500">
        ...
        <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" fill="currentColor" viewBox="0 0 16 16" style="margin-left:auto;color:var(--theme-border,#e6e6e6)"><path fill-rule="evenodd" d="M4.646 1.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1 0 .708l-6 6a.5.5 0 0 1-.708-.708L10.293 8 4.646 2.354a.5.5 0 0 1 0-.708z"/></svg>
      </a>
    <?php endforeach; ?>
    <?php if (!$hasSocialLink): ?>
      <span style="font-size:.8rem;color:var(--theme-muted,#757575)">Brak skonfigurowanych linków</span>
    <?php endif; ?>

  <?php elseif ($_swType === 'custom_html'): ?>
    <div style="font-size:.875rem;line-height:1.7;color:var(--theme-text,#292929)">
      <?= $_swData['html'] ?? '' ?>
    </div>

  <?php else: ?>
    <span style="font-size:.8rem;color:var(--theme-muted,#757575)">Nieznany typ widgetu: <?= htmlspecialchars($_swType) ?></span>
  <?php endif; ?>
</div>
<?php endforeach; ?>
