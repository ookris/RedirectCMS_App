<?php
/* Shared partial: 4-column footer + scripts + </body></html>.
 * Dołączany jako ostatni element przez home.php / post.php / page.php / contact.php.
 * Zmienne: $homeTitle, $homeSubtitle, $homeFooter, $basePath, $blogPosts,
 *          $allCategories, $navPages, $contactEnabled, $socialLinks,
 *          $lightboxEnabled, $homeFooterCode
 */
$_footerSvgPaths = [
...
// Ostatnie wpisy przekazywane przez index.php (getRecentPosts) — dostępne na każdej podstronie
$_recentPosts = $recentPosts ?? [];
$_footerCats  = array_slice($allCategories ?? [], 0, 8);
?>
...
<footer class="site-footer">
  <div class="container">
    <div class="row g-5">
      ...
      <!-- Col 2: Ostatnie wpisy -->
      <div class="col-12 col-md-6 col-lg-3">
        <h4 class="footer-widget-title">Ostatnie wpisy</h4>
        <?php if (!empty($_recentPosts)): ?>
          <?php foreach ($_recentPosts as $_rp): ?>
            <?php
              $_rImg = !empty($_rp['og_image']) ? $basePath . '/' . ltrim((string)$_rp['og_image'], '/') : null;
              $_rLetter = mb_strtoupper(mb_substr((string)($_rp['page_title'] ?? 'P'), 0, 1));
            ?>
            <a href="<?= $basePath ?>/blog/<?= htmlspecialchars($_rp['slug']) ?>"
               class="footer-recent-post">
              <?php if ($_rImg): ?>
                <img src="<?= htmlspecialchars($_rImg) ?>"
                     alt="<?= htmlspecialchars($_rp['page_title'] ?? '') ?>"
                     class="footer-recent-thumb" loading="lazy">
              <?php else: ?>
                <div class="footer-recent-thumb-placeholder"><?= htmlspecialchars($_rLetter) ?></div>
              <?php endif; ?>
              <div>
                <div class="footer-recent-title">
                  <?= htmlspecialchars(mb_substr((string)($_rp['page_title'] ?? $_rp['slug']), 0, 52)) ?>
                </div>
                <div class="footer-recent-date">
                  <?= htmlspecialchars(substr((string)($_rp['created_at'] ?? ''), 0, 10)) ?>
                </div>
              </div>
            </a>
          <?php endforeach; ?>
        <?php else: ?>
          <p class="small" style="opacity:.5">Brak wpisów.</p>
        <?php endif; ?>
      </div>

      <!-- Col 3: Kategorie -->
      <div class="col-12 col-md-6 col-lg-3">
        <h4 class="footer-widget-title">Kategorie</h4>
        <?php if (!empty($_footerCats)): ?>
          <?php foreach ($_footerCats as $_fc): ?>
            <a href="<?= !empty($prettyUrls) ? $basePath . '/category/' . urlencode($_fc['slug']) : $basePath . '/?category=' . urlencode($_fc['slug']) ?>"
               class="footer-cat-link">
              <span class="footer-cat-dot"
                    style="background:<?= htmlspecialchars($_fc['color'] ?? '#6c757d') ?>"></span>
              <?= htmlspecialchars($_fc['name']) ?>
              <?php if (!empty($_fc['post_count'])): ?>
                <span style="opacity:.4; font-size:.72rem; margin-left:auto">(<?= (int)$_fc['post_count'] ?>)</span>
              <?php endif; ?>
            </a>
          <?php endforeach; ?>
        <?php else: ?>
          <p class="small" style="opacity:.5">Brak kategorii.</p>
        <?php endif; ?>
      </div>

      <!-- Col 4: Szybkie linki -->
      <div class="col-12 col-md-6 col-lg-3">
        <h4 class="footer-widget-title">Szybkie linki</h4>
        <a href="<?= $basePath ?>/" class="footer-link-item">Strona główna</a>
        <a href="<?= $basePath ?>/" class="footer-link-item">Blog</a>
        ...
        <?php if (!empty($contactEnabled)): ?>
          <a href="<?= $basePath ?>/?page=contact" class="footer-link-item">Kontakt</a>
        <?php endif; ?>
      </div>
  ...
  <div class="footer-bottom">
    <div class="container d-flex flex-wrap justify-content-between align-items-center gap-2">
      ...
      <span style="opacity:.4">Powered by RedirectCMS</span>
    </div>
  </div>
</footer>
