<?php
// _sidebar.php — JTemplate sidebar widgets
// Variables available: $sidebarData, $basePath, $prettyUrls
// Uses jt_* helper functions defined in parent template (home.php / post.php)
...
<div class="jt-widget">
  <?php if (!empty($_swTitle)): ?>
    <div class="jt-widget-title"><?= htmlspecialchars($_swTitle) ?></div>
  <?php endif; ?>

  <?php if ($_swType === 'popular_posts' || $_swType === 'random_posts'): ?>
    <?php foreach ($_swData as $_i => $_wp): ?>
      <a href="<?= jt_post_url($basePath, $_wp['slug'], $prettyUrls ?? false) ?>"
         style="display:flex;align-items:flex-start;gap:10px;padding:10px 0;border-bottom:1px solid var(--jt-border);text-decoration:none;color:var(--jt-text);transition:color .15s"
         onmouseover="this.style.color='var(--jt-accent)'" onmouseout="this.style.color='var(--jt-text)'">
        <span style="flex-shrink:0;width:26px;height:26px;border-radius:6px;
              background:<?= $_i === 0 ? 'var(--jt-accent)' : 'var(--jt-border)' ?>;
              color:<?= $_i === 0 ? '#fff' : 'var(--jt-muted)' ?>;
              font-size:.68rem;font-weight:700;display:flex;align-items:center;justify-content:center;font-family:var(--jt-font)">
          <?= $_i + 1 ?>
        </span>
        <span style="flex:1;min-width:0">
          ...
          <?php if (!empty($_wp['click_count'])): ?>
            <span style="font-size:.69rem;color:var(--jt-muted);margin-top:3px;display:block">
              <svg width="10" height="10" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" style="display:inline;vertical-align:middle;margin-right:3px"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
    ...
    <?php if (empty($_swData)): ?>
      <span style="font-size:.8rem;color:var(--jt-muted)">Brak wpisów</span>
    <?php endif; ?>

  <?php elseif ($_swType === 'tag_cloud'): ?>
    <div style="display:flex;flex-wrap:wrap;gap:6px">
      <?php foreach ($_swData as $_tag): ?>
        <a href="<?= jt_tag_url($basePath, $_tag['slug'], $prettyUrls ?? false) ?>"
           style="display:inline-block;padding:4px 12px;border-radius:99px;font-size:.73rem;font-weight:600;
                  background:color-mix(in srgb,var(--jt-accent) 10%,transparent);
                  color:var(--jt-accent);
                  border:1px solid color-mix(in srgb,var(--jt-accent) 25%,transparent);
                  text-decoration:none;transition:background .15s,color .15s"
           onmouseover="this.style.background='var(--jt-accent)';this.style.color='#fff'"
           onmouseout="this.style.background='color-mix(in srgb,var(--jt-accent) 10%,transparent)';this.style.color='var(--jt-accent)'">
          #<?= htmlspecialchars($_tag['name']) ?>
        </a>
      <?php endforeach; ?>
      <?php if (empty($_swData)): ?>
        <span style="font-size:.8rem;color:var(--jt-muted)">Brak tagów</span>
      <?php endif; ?>
    </div>
  ...
  <?php elseif ($_swType === 'categories'): ?>
    <?php foreach ($_swData as $_cat): ?>
      <a href="<?= jt_cat_url($basePath, $_cat['slug'], $prettyUrls ?? false) ?>"
         style="display:flex;justify-content:space-between;align-items:center;padding:9px 0;
                border-bottom:1px solid var(--jt-border);font-size:.84rem;color:var(--jt-text);
                text-decoration:none;transition:color .15s"
         onmouseover="this.style.color='var(--jt-accent)'" onmouseout="this.style.color='var(--jt-text)'">
        <span style="display:flex;align-items:center;gap:8px">
          <span style="width:8px;height:8px;border-radius:50%;flex-shrink:0;
                background:<?= htmlspecialchars($_cat['color'] ?: 'var(--jt-accent)', ENT_QUOTES) ?>">
          </span>
          <?= htmlspecialchars($_cat['name']) ?>
        </span>
        <span style="font-size:.68rem;color:var(--jt-muted);background:var(--jt-border);
    ...
    <?php endforeach; ?>
    <?php if (empty($_swData)): ?>
      <span style="font-size:.8rem;color:var(--jt-muted)">Brak kategorii</span>
    <?php endif; ?>
    ...
    ?>
      <a href="<?= htmlspecialchars($url, ENT_QUOTES) ?>" target="_blank" rel="noopener noreferrer"
         style="display:flex;align-items:center;gap:10px;padding:9px 0;
                border-bottom:1px solid var(--jt-border);text-decoration:none;
                color:var(--jt-text);font-size:.84rem;font-weight:500;transition:color .15s"
         onmouseover="this.style.color='var(--jt-accent)'" onmouseout="this.style.color='var(--jt-text)'">
        <span style="width:30px;height:30px;border-radius:7px;
              background:<?= htmlspecialchars($_def['color'], ENT_QUOTES) ?>;
              color:#fff;display:flex;align-items:center;justify-content:center;flex-shrink:0">
          <?= $_def['svg'] ?>
        </span>
        <?= htmlspecialchars($_def['label']) ?>
        <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24" style="margin-left:auto;color:var(--jt-border)"><path d="M9 18l6-6-6-6"/></svg>
      </a>
    <?php endforeach; ?>
    <?php if (!$hasSocial): ?>
      <span style="font-size:.8rem;color:var(--jt-muted)">Brak skonfigurowanych linków</span>
    <?php endif; ?>

  <?php elseif ($_swType === 'custom_html'): ?>
    <div style="font-size:.875rem;line-height:1.7;color:var(--jt-text)">
      <?= $_swData['html'] ?? '' ?>
    </div>

  <?php else: ?>
    <span style="font-size:.8rem;color:var(--jt-muted)">Nieznany typ widgetu: <?= htmlspecialchars($_swType) ?></span>
  <?php endif; ?>
</div>
<?php endforeach; ?>
